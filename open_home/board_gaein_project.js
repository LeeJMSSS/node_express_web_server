...
router.get('/', function (req, res) {
    res.redirect('/open_home/gaein_project/list/1');
});
...
router.post('/write', upload.array('file'), function (req, res) {
    ...
    if (files.length > 0) {
        var directory = './uploads/open_home/board/gaein_project/' + cur_date + "_" + req.session.user.name + "/";
        mkdir(directory);
    ...
    pool.getConnection(function (err, connection) {
        // Use the connection
        var sqlForInsertBoard = "insert into gaein_project(name,password,title,text,filename ,date ) values(?,?,?,?,?,?);";

        connection.query(sqlForInsertBoard, datas, function (err, rows) {
            if (err) console.error("err : " + err);
            res.redirect('/open_home/gaein_project');
            connection.release();
...
router.get('/list/:page', function (req, res, next) {
    ...
    pool.getConnection(function (err, connection) {
        var query = "SELECT max(no) as max_id FROM gaein_project;"
        connection.query(query, function (err, count) {
            ...
            query = "SELECT no,title, name ,replys,date,hit FROM gaein_project where no >" + start_no + " AND  no<=" + end_no + ";";
...
var update_hit = function (no, hit) {
    ...
    pool.getConnection(function (err, connection) {
        hit = hit + 1;
        var query = "update gaein_project " + "set hit=" + hit + " where no=" + no;
        connection.query(query, function (err, rows) {
            if (err) console.error(err);
            connection.release();
        });
...
var update_comment = function (no, replys) {
    ...
    pool.getConnection(function (err, connection) {
        var query = "update gaein_project " + "set replys=" + replys + " where no=" + no;
        connection.query(query, function (err, rows) {
            if (err) console.error(err);
            connection.release();
        });
...
router.get('/read/:idx', function (req, res) {
    ...
    pool.getConnection(function (err, connection) {
        var query = "SELECT no,name,title,text,hit,filename FROM gaein_project where no=" + req.params.idx;
        connection.query(query, function (err, rows) {
            var files = "";
            if (err) console.error(err);
            connection.release();
            ...
            res.render('./open_home/board_read', {
                ...
                idx: req.params.idx,
                user_name: req.session.user.name,
                file_url: "/open_home/gaein_project/download",
                files: files
            });
        });
    });
...
router.get('/download/:idx/:path', function (req, res) {
    ...
    pool.getConnection(function (err, connection) {
        var query = "SELECT filename FROM gaein_project where no=" + idx;
        connection.query(query, function (err, rows) {
            if (err) console.error(err);
            connection.release();
