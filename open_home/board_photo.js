...
var update_hit = function (no, hit) {
    pool.getConnection(function (err, connection) {
        hit = hit + 1;
        var query = "update board_photo " + "set hit=" + hit + " where no=" + no;
...
var update_comment = function (no, comment) {
    pool.getConnection(function (err, connection) {
        hit = hit + 1;
        var query = "update board_photo " + "set replys=" + replys + " where no=" + no;
        connection.query(query, function (err, rows) {
            if (err) console.error(err);
            connection.release();
        });
...
router.get('/', function (req, res) {
    res.redirect('/open_home/photo/list/1');
});
router.get('/list/:page', function (req, res, next) {
    ...
    pool.getConnection(function (err, connection) {
        var query = "SELECT max(no) as max_id FROM board_photo;"
        connection.query(query, function (err, count) {
            ...
            query = "SELECT no,title, name ,replys,date,hit FROM board_photo where no >" + start_no + " AND  no<=" + end_no + ";";
            connection.query(query, function (err, rows) {
                if (err) console.error(err);
...
router.get('/write', function (req, res, next) {
    ...
    res.render('./open_home/board_photo_write', {
        title: "사진",
        position: "photo",
...
router.post('/write', upload.array('file'), function (req, res) {
    ...
    if (files.length > 0) {
        var directory = './uploads/open_home/board/photo/' + cur_date + "_" + req.session.user.name + "/";
        mkdir(directory);
    ...
    pool.getConnection(function (err, connection) {
        // Use the connection
        var sqlForInsertBoard = "insert into board_photo(name,password,title,text,filename ,date ) values(?,?,?,?,?,?);";

        connection.query(sqlForInsertBoard, datas, function (err, rows) {
            if (err) console.error("err : " + err);
            res.redirect('/open_home/photo');
            connection.release();
...
router.get('/read/:idx', function (req, res) {
    ...
    pool.getConnection(function (err, connection) {
        var query = "SELECT no,name,title,text,hit,filename FROM board_photo where no=" + req.params.idx;
        connection.query(query, function (err, rows) {
            var files = "";
            console.log('ddd');
            if (err) console.error(err);
            ...
            res.render('./open_home/board_read', {
                ...
                idx: req.params.idx,
                user_name: req.session.user.name,
                file_url: "/open_home/photo/download",
                files: files
            });
        });
    });
